// Given an array nums of 0s and 1s and an integer k, return True
...
var kLengthApart = function (nums, k) {
  ...
  let gap = [];
  for (let j = 1; j < indexOfOne.length; j++) {
    gap.push(indexOfOne[j] - indexOfOne[j - 1]);
  }
  let minGap = gap.sort()[0];
  if (k >= minGap) {
    return false;
  } else {
    return true;
