...
// A value arr[i] is said to be stronger than a
// value arr[j] if | arr[i] - m | > | arr[j] - m | where m is the median of the array.
...
var getStrongest = function (arr, k) {
  ...
  let index = [];
  let final = [];
  const indexOfMedian = Math.floor((arr.length - 1) / 2);
  const median = arr[indexOfMedian];
  arr.map((el) => {
    result.push({ origin: el, gap: el - median });
  });
  for (let j = 0; j < k; j++) {
    for (let i = 0; i < result.length; i++) {
      if (result[i].gap < 0) {
        result[i].gap = -result[i].gap;
        index.push(result[i].gap);
      } else {
        result[i].gap;
        index.push(result[i].gap);
      }
      ...
      if (result[i].gap === index[k]) {
        if (final.length < k) {
          final.push(result[i].origin);
        }
      }
    }
  }
